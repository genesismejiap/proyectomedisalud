<div style="margin-bottom:1.5rem;">
    <h2 style="color:#0284c7; margin-bottom:.25rem;"><i class="fas fa-history"></i> Histórico de Compras - Mis Transacciones</h2>
    <p style="color:#64748b; margin:0;">Registro histórico de todas las compras realizadas en Farmacia mediSalud.</p>
</div>
    ...
    <table class="medisalud-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{total_items}} prod.</td>
                <td>L. {{txnisv}}</td>
                <td><strong>L. {{txntotal}}</strong></td>
                <td><span class="product-stock-badge stock-ok"><i class="fas fa-check"></i> {{txnstt}}</span></td>
                <td style="text-align:right;">
                    <a href="index.php?page=Transacciones_MisTransacciones&txnid={{txnid}}" class="btn btn-outline btn-sm">
...
<div style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15,23,42,0.6); display:flex; align-items:center; justify-content:center; z-index:9999; backdrop-filter:blur(4px);">
    <div style="background:#ffffff; width:90%; max-width:650px; border-radius:1.5rem; padding:2rem; max-height:90vh; overflow-y:auto; box-shadow:0 20px 40px rgba(0,0,0,0.2);">
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #e2e8f0; padding-bottom:1rem; margin-bottom:1rem;">
            <h3 style="margin:0; color:#0284c7;"><i class="fas fa-file-invoice"></i> Detalle de Transacción</h3>
            <a href="index.php?page=Transacciones_MisTransacciones" style="font-size:1.5rem; color:#64748b; text-decoration:none;">&times;</a>
        </div>

        {{with selectedTxn}}
        <div style="background:#f8fafc; padding:1rem; border-radius:.75rem; margin-bottom:1rem; font-size:.9rem;">
        ...
        <h4 style="margin-bottom:.5rem; color:#0f172a;">Productos Comprados:</h4>
        ...
        <div style="text-align:right; font-size:1.1rem; color:#0369a1; padding-top:.5rem; border-top:2px solid #e2e8f0;">
            <div>Subtotal: L. {{txnamount}}</div>
            <div>ISV (15%): L. {{txnisv}}</div>
            <div style="font-size:1.3rem; font-weight:800; color:#0284c7; margin-top:.25rem;">Total: L. {{txntotal}}</div>
        </div>
        {{endwith selectedTxn}}

        <div style="margin-top:1.5rem; text-align:right;">
            <a href="index.php?page=Transacciones_MisTransacciones" class="btn btn-primary">Cerrar</a>
        </div>
    </div>
</div>
{{endif hasSelected}}
